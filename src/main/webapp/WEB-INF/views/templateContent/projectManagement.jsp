	...
	$(document)
			.ready(
					function() {
													...
													paginatedData
															.forEach(function(
																...
																let removeButton = $(
																		"<button>")
																		.addClass(
																				"btn btn-danger deleteUsersProject")
																		.text(
																				"추방");
																removeButtonCell
																		.append(removeButton);
																row
																		.append(removeButtonCell);
																tableBody
																		.append(row);
															});
												}
							...
							$
									.ajax({
										...
										success : function(response) {
											...
											let endIndex = startIndex
													+ pageSize;
											let paginatedData = response.slice(
													startIndex, endIndex);
						...
						function addUserToTable(user) {
							...
							if (Array.isArray(user)) {
								for (let i = 0; i < user.length; i++) {
									...
									let nameCell = $("<td>").text(
											currentUser.name);
									row.append(nameCell);

									let userIdCell = $("<td>").text(
											currentUser.userid);
									...
									let inviteButton = $("<a>")
											.addClass(
													"btn btn-outline-primary userInvite")
											.text("초대");
						...
						// 초대한 유저 참가자에 비동기적 추가
						async
						function updateProjectParticipantsAsync() {
							try {
								$
										.ajax({
											url : "member/users/" + projectId,
